...
C<DEBUG> is a constant (similar to C<use constant>) which holds your current debugging level.
Because it's implemented using constant folding, any conditional based on it will actually be
...
=head2 debuggit([level =>] arg[, ...])

debuggit() is the only exported function of Debuggit (except for the constant, which is technically
a function, but you know what I mean).  When DEBUG is set to 0, you are guaranteed that C<debuggit>
...
=head2 [local] $Debuggit::formatter = coderef
...
Before you try to get too terribly fancy with the formatter, you may wish to investigate add_func().
A good rule of thumb when trying to decide whether you want a new function or a new formatter is
this: imagine that debuggit() takes a bunch of chunks of text and produces a line (which is pretty
much what is does).  If you want to fiddle with how one of those chunks looks, you want a function.
If you need to change the look of the whole line, though, you need a new formatter.

If you're more interested in where the formatted output gets sent to, look at $output.

=head2 Debuggit::default_formatter(@_)

This function is what C<$formatter> (see above) is set to unless (and until) you change it.  It can
also be called from your own formatter function (again, see above).  Its purpose is to turn the
arguments you pass C<debuggit()> into a formatted line.  This line is then sent to the function
stored in $output.
...
=head2 [local] $Debuggit::output = coderef
...
perl(1), L<Log::Log4perl>, L<debug>, L<Debug>, L<Debug::Message>, L<Debug::EchoMessage>.
